...
## Transfer Learning
- warm starting 으로 중요 데이터에 대해 1차 학습하여, 중요 포인트를 먼저 학습, 2차로, negative도 포함하여 학습
- 아무래도 처음부터 전체데이터로 학습을 하면 “O”외에 다른 엔티티의 빈도가 낮아서 학습할 기회(?)를 잃어 학습이 잘 되지 않는 반면, warm starting을 하면 1차로 transaction데이터로 학습을 하면 모델이 transaction에 있는 엔티티를 학습할 수 있었고, 2차로 전체데이터를 학습하며 네거티브 케이스(transaction화 되지 않는 데이터)도 학습을 하지 않았을까 생각
- Recommendation model 학습할 때도 New user 제외하고 최소 item 5건 이상 구매 경험 유저만으로 학습을 먼저 많이들 한다
- 학교 공부로 치자면,
  1차로 중요 핵심 요점 정리로 과외를 받아, 어떤게 중요한지 모델이 알 수 있게 하고,
  2차로 모의고사 기출문제로 학습을 해서, 예외케이스나 애매한 부분에 대해서 학습을 한 것으로
  보면 될까요? 처음부터 전체를 공부하면, 사람도, 뭐가 뭔지 잘 모르니까요.
- 다른 방식으로, 실제 서빙보다 더 일반적으로 학습하고, 실제 서빙시 유저 대상으로 다시 심화 학습하는 방식도 있음
